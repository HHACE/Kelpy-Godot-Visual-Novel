...
func _select_show():
	match P1_select_index:
		...
		4:
			$Select/SelectIcon.position = Vector2(525, $exit/exit.position.y + 55) 
			$exit/exit.grab_focus()
		_:
			print("Default case: Invalid selection")
...
func save_volume():
	...
	file.store_string(JSON.stringify({"volume": master_volume}))
	file.close()
